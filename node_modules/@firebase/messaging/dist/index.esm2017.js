...
class SubscriptionManager {
    async getToken(app, subscription, vapidKey) {
     ...
     */
    async updateToken(tokenDetails, app, subscription, vapidKey) {
        const headers = await getHeaders(app);
        const body = getBody(subscription, vapidKey);
        const updateOptions = {
            method: 'PATCH',
        ...
        let responseData;
        try {
            const response = await fetch(`${getEndpoint(app)}/${tokenDetails.fcmToken}`, updateOptions);
            responseData = await response.json();
        }
        catch (err) {
    ...
    async deleteToken(app, tokenDetails) {
        // TODO: Add FIS header
        const headers = await getHeaders(app);
        ...
        try {
            const response = await fetch(`${getEndpoint(app)}/${tokenDetails.fcmToken}`, unsubscribeOptions);
...
class BaseController {
    ...
    async updateToken(swReg, pushSubscription, publicVapidKey, tokenDetails) {
        try {
            const updatedToken = await this.subscriptionManager.updateToken(tokenDetails, this.app, pushSubscription, publicVapidKey);
            const allDetails = {
                swScope: swReg.scope,
    ...
    async getNewToken(swReg, pushSubscription, publicVapidKey) {
        const newToken = await this.subscriptionManager.getToken(this.app, pushSubscription, publicVapidKey);
        const allDetails = {
            swScope: swReg.scope,
            vapidKey: publicVapidKey,
            fcmSenderId: this.app.options.messagingSenderId,
    ...
    setupSWMessageListener_() {
        navigator.serviceWorker.addEventListener('message', event => {
            ...
            if (data &&
                ...
                this.app.INTERNAL.analytics.logEvent(eventType, 
