...
jasmine_util_1.describeWithFlags('profiler.Profiler', jasmine_util_1.SYNC_BACKEND_ENVS, function () {
    it('profiles simple function', function (doneFn) {
        ...
        profiler.profileKernel('MatMul', inputs, function () {
            kernelCalled = true;
            return [resultScalar];
        });
        setTimeout(function () {
    ...
    it('profiles nested kernel', function (doneFn) {
        ...
        var resultScalar = tf.scalar(result);
        profiler.profileKernel('MatMul', inputs, function () {
            var result = profiler.profileKernel('Max', inputs, function () {
...
describe('profiler.checkComputationForErrors', function () {
    it('Float32Array has NaN', function () {
        expect(profiler_1.checkComputationForErrors(new Float32Array([1, 2, 3, NaN, 4, 255]), 'float32', 'test'))
            .toBe(true);
    });
    it('Float32Array has Infinity', function () {
        expect(profiler_1.checkComputationForErrors(new Float32Array([1, 2, 3, Infinity, 4, 255]), 'float32', 'test'))
            .toBe(true);
    });
    it('Float32Array no NaN', function () {
        // Int32 and Bool NaNs should not trigger an error.
        expect(profiler_1.checkComputationForErrors(new Float32Array([1, 2, 3, -1, 4, 255]), 'float32', 'test'))
            .toBe(false);
    });
});
